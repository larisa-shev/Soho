...
// 1. Імпортуємо файл (Vite сам зробить з нього об'єкт)
import appartsData from './apparts.json';

export async function initInteractiveMaps() {
	try {
		const data = appartsData;
		const containers = document.querySelectorAll('.js-svg-container');

		containers.forEach((container, index) => {
			const floorFileName = container.dataset.floor;
			const mapData = data.find(item => item.file_name === floorFileName);

			if (mapData) {
				// Визначаємо будинок
				const houseEl = container.closest('#East, #West, .tabs__content');
				const housePrefix = houseEl?.id || (container.closest('.tabs__content')?.dataset.index === "0" ? 'East' : 'West');

				// Створюємо SVG з унікальним індексом для ID
				const svg = createSVG(mapData, housePrefix, index);
				container.innerHTML = '';
		...
		setTimeout(() => {
			document.querySelectorAll('.swiper').forEach(s => s.swiper && s.swiper.update());
		}, 400);
	} catch (error) {
		console.error(error);
	}
...
function createSVG(data, housePrefix, index) {
	const svgNS = "http://www.w3.org/2000/svg";
	const svg = document.createElementNS(svgNS, "svg");

	// Створюємо ID, який НІКОЛИ не повториться (додаємо index)
	const uniquePatternId = `pattern_${housePrefix}_${index}_${data.route.floor}`;

	svg.id = `${housePrefix}_${data.route.floor}`; // Для CSS (#East_5)
	svg.setAttribute("class", "floor house_floor");
	svg.setAttribute("viewBox", `${data.viewBox.x} ${data.viewBox.y} ${data.viewBox.width} ${data.viewBox.height}`);

	// Оновлюємо внутрішній ID паттерна в рядку з JSON
	// Ми шукаємо старий ID (напр. bg_0_floor) і міняємо на наш унікальний
	let updatedDefs = data.defs.replace(/id=".*?"/, `id="${uniquePatternId}"`);

	const defsElement = document.createElementNS(svgNS, "defs");
	defsElement.innerHTML = updatedDefs;
	svg.appendChild(defsElement);

	// Створюємо фон, який посилається на наш унікальний ID
	const rect = document.createElementNS(svgNS, "rect");
	rect.setAttribute("width", "100%");
	rect.setAttribute("height", "100%");
	rect.setAttribute("fill", `url(#${uniquePatternId})`);
	svg.appendChild(rect);

	data.polygons.forEach(poly => {
		const g = document.createElementNS(svgNS, "g");
		g.setAttribute("class", "apt");
		g.setAttribute("fill", `url(#${uniquePatternId})`); // Квартира теж має цей малюнок

		// Встановлюємо центр збільшення
		if (poly.box && poly.box.origin) {
			g.style.transformOrigin = poly.box.origin.join(' ');
		}

		const polygon = document.createElementNS(svgNS, "polygon");
		polygon.setAttribute("points", poly.points.join(','));
